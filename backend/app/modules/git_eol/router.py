...
def enqueue_preview(
    ...
    user: User = Depends(require_menu_permission("git_eol")),
...
def fetch_job(job_id: str, user: User = Depends(require_menu_permission("git_eol"))) -> GitEolJobResponse:
...
def fetch_job_logs(job_id: str, user: User = Depends(require_menu_permission("git_eol"))) -> list[GitEolJobLog]:
    payload = get_job(job_id)
    if not payload:
        raise HTTPException(status_code=404, detail="Job not found")
    if payload.get("user_id") != user.id:
