...
public sealed class WordService : IFileHandler
{
    ...
    {
        using var trace = DebugTrace.Enter("WordService", "ImportAsync", () => new
        {
            format = "Word",
            profile = "office-v1",
        ...
        try
        {
            ...
            trace.State("source", () => new { digest = source.SourceHash, size = source.OriginalBytes.Length, format = "Word" });
            ...
            trace.State("plan", () => new { unitCount = plan.Units.Count, tableCount = plan.Tables.Count });
    ...
    {
        using var trace = DebugTrace.Enter("WordService", "ExportAsync", () => new
        {
            format = "Word",
            profile = "office-v1",
            sourceCanRead = stream.CanRead,
