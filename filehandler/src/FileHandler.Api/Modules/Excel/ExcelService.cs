...
public sealed class ExcelService : IFileHandler
{
    ...
    {
        using var trace = DebugTrace.Enter("ExcelService", "ImportAsync", () => new
        {
            format = "Excel",
            profile = "office-v1",
        ...
        try
        {
            ...
            trace.State("source", () => new { digest = source.SourceHash, size = source.OriginalBytes.Length, format = "Excel" });
            ...
            trace.State("plan", () => new { unitCount = plan.Units.Count, sheetCount = plan.Sheets.Count });
    ...
    {
        using var trace = DebugTrace.Enter("ExcelService", "ExportAsync", () => new
        {
            format = "Excel",
            profile = "office-v1",
            sourceCanRead = stream.CanRead,
